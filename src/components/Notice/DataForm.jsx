...
function DataForm({ handleSubmit, handleFileChange, fileName, data, setData, setFileName, children }) {
  const id = useId()
  // 제목 변경 onChange 핸들러
  const handleTitleChange = (event) => {
    const newTitle = event.target.value
    setData({ ...data, noticeTitle: newTitle })
  }

  // 내용 변경 onChange 핸들러
  const handleDescriptionChange = (event) => {
    const newDescription = event.target.value
    setData({ ...data, noticeDescription: newDescription })
  }
