...
function TabContents({ data, searchResult }) {
  ...
  const filteredData = searchResult ? data?.items?.filter((item) => item.title.includes(searchResult)) : data?.items
              ...
              <div key={item.id} className="w-full font-light bg-white border cursor-pointer h-fit rounded-2xl text-deepDarkGray ">
                ...
                >
                  <div className="overflow-hidden rounded-t-2xl">
                    <LazyImage src={getPbImageURL(item, "thumbnail")} alt={item.title} className="object-cover rounded-t-2xl hover:scale-[1.1] hover:translate-[50%] transition-all" />
                  </div>
